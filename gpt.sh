...
# Ustawienia ochrony dla plików i folderów
...
# Usunięcie standardowych plików WordPress, które będą przywrócone
...
# Proces weryfikacji pliku wp-config.php
...
# Sprawdzenie plików w głównym katalogu
...
for folder in $(find -maxdepth 1 -type d ! -name 'wp-content' | sed -r '/^\.$/d'); do 
    while true; do
        ...
        case $yn in 
            [yY] ) echo 'usuwam '
                   rm -rf $folder
                   echo "Usuwam folder $folder" >> $log_file
                   break;;
...
# Wgranie świeżych plików WordPress
...
# Włączenie automatycznych aktualizacji dla pluginów
...
# Instalacja i aktywacja pluginu Wordfence
...
# Usunięcie podejrzanych plików PHP w katalogu uploads
...
# Usunięcie zainfekowanych plików PHP.VIRUS w katalogu uploads
...
# Sprawdzenie i czyszczenie kont administratorów
...
for user in $($wp_path user list --field=ID --role=administrator --skip-plugins --skip-themes); do
    while true; do
        ...
        case $yn in
            [yY] ) echo "usuwam użytkownika $user_email. W kolejnym kroku będzie trzeba podać ID użytkownika, któremu przypiszemy posty"
                   $wp_path user delete $user --prompt=reassign
                   break;;
...
# Reset wszystkich haseł administratorów
...
# Przywrócenie .htaccess do stanu fabrycznego
...
# Sprawdzenie innych plików .htaccess
...
do
    ...
    case $yn in 
        [yY] ) echo "usuwam pliki podobne do $name / ilosc plikow $ilosc_plikow" >> $log_file
               find -name .htaccess -size "${size}c" -delete
               continue;;
...
# Czyszczenie resztek po automatycznym usuwaniu wirusów
